...
def figures(out, frame, songs, genres, yearly_table, availability_table):
    ...
    def save(fig, name):
        fig.tight_layout()
        fig.savefig(dest / (name + '.png'), metadata={'Software': VERSION})
        plt.close(fig)
    # Same bins and density scale distinguish weighting without unequal-N distortion.
    for c in HISTOGRAMS:
        fig, ax = plt.subplots(figsize=(7, 4))
        x = frame[c].dropna()
        bins = np.arange(.5, 101.5, 1) if c == 'monthly_rank' else np.linspace(0, 1, 41) if c in FEATURES else 40
        _, edges, _ = ax.hist(x, bins=bins, density=True, alpha=.5, label=f'Song-months (n={len(x):,})')
        if c in songs:
            ax.hist(songs[c].dropna(), bins=edges, density=True, histtype='step', linewidth=1.5,
                    label=f'Songs (n={songs[c].notna().sum():,})')
        ax.set(xlabel=label(c), ylabel='Density', title=label(c) + ' distribution')
        if c == 'monthly_rank':
            ax.set_xlabel('Monthly rank (1 = best; 100 = worst)')
        ax.legend(fontsize=8)
        save(fig, 'hist_' + c)
    fig, ax = plt.subplots(figsize=(9, 5))
    ax.boxplot([songs[c].dropna() for c in SELECTED], vert=False, tick_labels=[label(c) for c in SELECTED],
               flierprops={'markersize': 1, 'alpha': .2})
    ax.set(xlabel='Model score (0–1)', title='Representative scores — each song once; 1.5 IQR whiskers')
    save(fig, 'song_score_boxplots')
    for c in ['ll_explicit_language', 'll_violence', 'sentiment_positive', 'sentiment_negative']:
        ...
        labels = [f'{g} (n={len(x):,})' + (' *' if len(x) < SMALL_SONGS else '') for g, x in zip(order, groups)]
        fig, ax = plt.subplots(figsize=(10, 7))
        ax.boxplot(groups, vert=False, tick_labels=labels, flierprops={'markersize': 1, 'alpha': .15})
        ax.set(xlabel=label(c) + ' (0–1)', title='By genre — each song once; * fewer than 30 available songs')
        save(fig, 'genre_' + c)
    fig, ax = plt.subplots(figsize=(10, 8))
    corr = frame[HEATMAP].corr(method='spearman')
    im = ax.imshow(corr, vmin=-1, vmax=1, cmap='RdBu_r')
    ax.set_xticks(range(len(HEATMAP)), [label(c) for c in HEATMAP], rotation=55, ha='right')
    ax.set_yticks(range(len(HEATMAP)), [label(c) for c in HEATMAP])
    ...
    save(fig, 'selected_spearman_heatmap')
    pairs = [('ll_explicit_language', 'monthly_rank'), ('sentiment_positive', 'monthly_rank'),
             ('ll_explicit_language', 'chart_weeks'), ('sentiment_negative', 'chart_weeks')] + AGREEMENT[:2]
    for a, b in pairs:
        ...
        ax.set(xlabel=label(a), ylabel=label(b), title=f'{label(a)} vs {label(b)}\n' +
               ('Song-months' if b == 'monthly_rank' else 'Each song once') + f'; paired n={len(pair):,}')
        if b == 'monthly_rank':
            ax.invert_yaxis()
            ax.set_ylabel('Monthly rank (1 = best; top of axis)')
        fig.colorbar(h, ax=ax, label='Count per hexagon (log scale)')
        save(fig, 'relationship_' + a + '_vs_' + b)
    fig, ax = plt.subplots(figsize=(9, 4.5))
    for measure in ['lyrics_available', 'all_42_classifier_scores', DURATION]:
        g = availability_table[(availability_table.group_dimension == 'decade') & (availability_table.measure == measure)]
        ax.plot(g['group'], 100-g.available_pct, marker='o', label=label(measure))
    ...
    ax.legend(fontsize=8)
    save(fig, 'missingness_by_decade')
    for name, columns in [('content', LL), ('sentiment_emotion', SELECTED[4:])]:
        fig, axes = plt.subplots(len(columns), 1, figsize=(10, 2.1*len(columns)), sharex=True)
        for ax, c in zip(axes, columns):
            for weighting, style in [('observation', '-'), ('song_within_year', '--')]:
                g = yearly_table[(yearly_table.variable == c) & (yearly_table.weighting == weighting)]
                ax.plot(g.year, g['mean'], style, label='Mean — ' + weighting.replace('_', ' '))
                if weighting == 'observation':
                    ax.plot(g.year, g['median'], ':', color='gray', label='Median — observation')
        ...
        axes[0].set_title('Annual descriptive scores; changing song mix and coverage; no hypothesis test')
        axes[-1].set_xlabel('Chart calendar year (not release year)')
        save(fig, 'annual_' + name)
