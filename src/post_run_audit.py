...
def main():
    ...
    fingerprint_matches = l.execute("SELECT value FROM production_settings WHERE key='pipeline_sha256'").fetchone()[0] == lyrics.fingerprint()
    summary=research_report.collect(c)
    counts=Counter(r['status'] for r in rows)
    meta=Counter(r['meta'] for r in rows)
    attempted=len(results)
    lines=['# Post-run acquisition audit', '', 'Generated by `python3 src/post_run_audit.py` from read-only local databases, evidence files, and caches. No requests or acquisition writes. See [audit decisions](../docs/post_run_audit.md) for run failure, validation commands, and next steps.',
    ...
    lines += ['', 'Calendar basket periods above include older songs appearing in those months; they differ from first-chart cohorts. These are availability diagnostics, not historical content indices.', '', '## Integrity and provenance', '',
              ...
              f'- Unified manifest statuses: `{dict(c.execute("SELECT lyrics_status,count(*) FROM lyrics_manifest GROUP BY lyrics_status"))}`. Sidecar is authoritative until a production-aware import.',
    ...
    target=ROOT/'reports/post_run_audit.md'
